...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email) || empty($department)) {
        ...
        $stmt = $conn->prepare("SELECT * FROM users WHERE name = ? AND email = ? AND department = ? AND role = 'teacher' LIMIT 1");
        ...
        if ($user) {
            ...
            $errors[] = "Invalid credentials. Please check your Full Name, Email and Department.";
        }
    }
}
?>
